...
class EncoderModel(pl.LightningModule):
    def __init__(self, learning_rate=1e-3, d_model=512, vocab_size=8000):
        super().__init__()
        ...
        self.encoder = nn.TransformerEncoder(
            self.encoder_layer, norm=nn.LayerNorm(self.d_model), num_layers=6
        )
        # self.linear = nn.Sequential(nn.Linear(self.d_emb, 2048), nn.ReLU(), nn.Linear(2048, self.d_emb))
        self.w = nn.Parameter(
